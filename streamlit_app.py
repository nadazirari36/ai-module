...
with st.sidebar:
    ...
    with col3:
        st.metric("Transactions", "397,924")
    
    st.markdown("---")
    
    st.header("🏆 Best Performance")
    st.metric("LSTM Precision@5", "32.23%", "6.4x better than random")
    st.metric("Ensemble Precision@5", "31.82%", "Nearly identical")
...
with col2:
    st.metric(
        ...
        value="31.82%",
        delta="-0.41%",
        help="Precision@5 - Nearly identical to LSTM"
    )
...
with col4:
    st.metric(
        ...
        value="2.71%",
        delta="-29.52%",
        help="Precision@5 - Struggles with sparse data"
    )
...
with col2:
    ...
    - **Higher is better**: 32% means 3 out of 10 recommendations match
    - **Random baseline**: 5% (guessing)
    - **Your system**: 32% (6.4x better!)
    """)
...
with st.expander("📚 System Architecture"):
    ...
    **Pipeline:**
    ...
    4. **Ensemble Fusion**: Average normalized scores
    5. **Ranking & Filtering**: Return top N products
    6. **Output**: Product recommendations with confidence scores
    """)

with st.expander("🧠 Model Details"):
    st.markdown("""
    **AutoEncoder (2.71% precision)**
    - Architecture: 3665→512→256→128→256→512→3665
    - Issue: Struggles with sparse binary matrices
    - Role: Captures general preferences
    
    **NCF - Neural Collaborative Filtering (17.20% precision)**
    - Embeddings: 64-dimensional user & item
    - Learns: Specific user-product chemistry
    - Advantage: Good baseline approach
    
    **LSTM - Long Short-Term Memory (32.23% precision)**
    - Architecture: LSTM(128) → Dense(64) → Dense(3665)
    - Learns: Sequential purchase patterns
    - Strength: Temporal dependencies matter!
    
    **Ensemble (31.82% precision)**
    - Method: Average normalized scores from all 3 models
    - Result: Nearly matches best single model
    - Benefit: Robust to individual weaknesses
    """)
...
with st.expander("📈 Evaluation Metrics"):
    ...
    **NDCG@5**: Ranking quality (punishes wrong recommendations more)
    
    **Comparison to Baselines:**
    - Random guessing: 5% precision
    - Show best-sellers: 8% precision
    - Your system: 32% precision (6.4x better!)
    """)
